...
namespace ArtifactSystemFixed
{
    ...
    {
        public static T ChooseWeightedProbability<T>(Dictionary<T, double> items, Random random, T otherwise = default)
        {
            var randomNum = random.NextDouble() * items.Values.Sum(); // [0, sum)
        ...
        public static int GetNumberOfArtifactFound(int artifactItemID)
        {
            ...
            if (arch.ContainsKey(artifactItemID))
                if (arch.TryGetValue(artifactItemID, out var value))
